    ...
    protected override Task ExecuteAsync(CancellationToken stoppingToken)
    {
        try
        {
            var factory = new ConnectionFactory
            {
                ...
                ConsumerDispatchConcurrency = 10, // allow processing 2 messages concurrently
                RequestedConnectionTimeout = TimeSpan.FromSeconds(5)
            };

            _connection = factory.CreateConnectionAsync("download-consumer", stoppingToken).GetAwaiter().GetResult();
            _channel    = _connection.CreateChannelAsync(null, stoppingToken).GetAwaiter().GetResult();
            _channel.QueueDeclareAsync(RabbitMqDownloadPublisher.QueueName,
                durable: true, exclusive: false, autoDelete: false).GetAwaiter().GetResult();
            _channel.BasicQosAsync(0, prefetchCount: 2, global: false).GetAwaiter().GetResult(); // process 2 concurrently

            var consumer = new AsyncEventingBasicConsumer(_channel);
            consumer.ReceivedAsync += OnMessageReceivedAsync;
            _channel.BasicConsumeAsync(RabbitMqDownloadPublisher.QueueName, autoAck: false, consumer);
